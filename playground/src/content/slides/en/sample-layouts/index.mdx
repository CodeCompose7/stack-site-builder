...
Math slides — inline · display · split with text
...
A fence standing alone on its own lines is a centered display block:
...
Mid-line it stays inline: the recursive form costs $$O(2^n)$$, the iterative one $$O(n)$$.

</Slide>

<Slide>

### Math left, text right
...
KaTeX runs at build time, so the slide ships plain HTML and no math runtime.

- it draws in `currentColor`, so light/dark needs no extra work
- a long derivation scrolls inside its own block instead of widening the slide
...
### A slide whose content borrows the chrome's attribute names

<div data-total="99" data-current="99" data-progress="99" data-next="99">
  <p>This block carries `data-total`, `data-current`, `data-progress` and
  `data-next`. The pager below must still read the real slide count, and this
  paragraph must survive: chrome lookups skip anything inside a slide.</p>
</div>

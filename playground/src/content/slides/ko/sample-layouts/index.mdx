...
펜스가 한 줄에 홀로 서면 가운데 정렬된 디스플레이 블록이 됩니다.
...
문장 안에 두면 인라인입니다. 재귀는 $$O(2^n)$$, 반복은 $$O(n)$$ 입니다.
...
KaTeX는 빌드 시점에 돌기 때문에 슬라이드에는 순수 HTML만 실리고 수식 런타임이 실리지 않습니다.

- `currentColor`로 그리므로 라이트·다크 전환에 손댈 것이 없습니다.
- 긴 전개는 슬라이드를 넓히지 않고 자기 블록 안에서 가로로 스크롤합니다.
